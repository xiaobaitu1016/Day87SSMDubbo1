    ...
    <div class="w900 fl">
        ...
        <form action="" method="post" class="layui-form layui-form-pane">
            <table class="layui-table" style="text-align: center;margin-top: 50px">
                <thead>
                <tr>
                    <th colspan="4"><center><h3><b>${doctor.dname} 个人信息修改</b></h3></center></th>
                </tr>
                </thead>
                <tbody>
                <tr>
                    <td>医生编号：</td>
                    <td width="270px">
                        <input id="did" value="DC${doctor.did+10000}" lay-verify="required" readonly="readonly" type="text" autocomplete="on" class="layui-input">
                    </td>
                    <td><b>医生姓名：</b></td>
                    <td width="270px">
                        <input id="dname" value="${doctor.dname}" lay-verify="required" type="text" autocomplete="on" class="layui-input">
                    </td>
                </tr>
                <tr>
                    <td>科室：</td>
                    <td>
                        <select id="dsid" lay-verify="required">
                            <c:forEach var="d" items="${allDbig}">
                                    <option></option>
                                    <optgroup label="${d.dbname}">
                                        <c:forEach items="${d.departmentsSmallList}" var="s">
                                            <c:if test="${s.dsid == doctor.dsid}">
                                                <option value="${s.dsid}" selected>${s.dsname}</option>
                                            </c:if>
                                            <c:if test="${s.dsid != doctor.dsid}">
                    ...
                    <td>
                        <select id="doctorrole" lay-filter="doctorrole" lay-verify="required">
                            <c:forEach items="${allDoctorRole}" var="s">
                                <c:if test="${s.drid == doctor.drid}">
                                    <option value="${s.drid}" selected>${s.drname}</option>
                                </c:if>
                                <c:if test="${s.drid != doctor.drid}">
                ...
                <tr>
                    <td>密码：</td>
                    <td width="270px">
                        <input id="pass" value="${doctor.password}" lay-verify="required" type="text" autocomplete="on" class="layui-input">
                    </td>
                    <td>好评数：</td>
                    <td width="270px">
                        <input id="praise" value="${doctor.praise}" readonly="readonly" lay-verify="required" type="text" autocomplete="on" class="layui-input">
                    </td>
                </tr>
                <tr>
                    <td>职位：</td>
                    <td colspan="3">
                        <div class="layui-inline">
                            <c:forEach var="i" items="${allRole}">
                                <c:if test="${fn:contains(rnameList, i.rname) }">
                                    <input type="checkbox" name="rid" value="${i.rid}" title="${i.rname}" checked="checked"/>
                                </c:if>
                                <c:if test="${!fn:contains(rnameList, i.rname) }">
                ...
                <tr>
                    <td>联系电话：</td>
                    <td colspan="3">
                        <input id="phone" lay-verify="required" value="${doctor.phone}" type="text" autocomplete="on" class="layui-input">
                    </td>
                </tr>
                <tr>
                    <td>擅长疾病：</td>
                    <td colspan="3">
                        <div class="layui-inline">
                            <c:forEach var="i" items="${allIllness}">
                                <c:if test="${fn:contains(illnessList, i.ilname) }">
                                    <input type="checkbox" name="ilid" value="${i.ilid}" title="${i.ilname}" checked="checked"/>
                                </c:if>
                                <c:if test="${!fn:contains(illnessList, i.ilname) }">
